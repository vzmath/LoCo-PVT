...
class MiT(nn.Module):
    ...
    def __init__(self, norm_cfg='BN', activation_cfg='ReLU', weight_std=False, img_size=[48, 192, 192], num_classes=None, in_chans=1, 
        ...
        if encoder=='small':
            ...
            self.transformer = MiT_encoder.model_medium(norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std, img_size=img_size, in_chans=in_chans)
        elif encoder=='large':
            self.transformer = MiT_encoder.model_large(norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std, img_size=img_size, in_chans=in_chans)
...
class MiTnet(SegmentationNetwork):
    ...
    def __init__(self, norm_cfg='BN', activation_cfg='ReLU', weight_std=False, img_size=None, num_classes=None, in_chans=1, 
        ...
        # embed_dims=[256,128,48,32], num_heads=[8, 4, 2], depths=[3, 12, 6], mlp_ratios=[4, 4, 4], sr_ratios=[2, 4, 6], encoder='large')
        
        # medium
        # self.model = MiT(norm_cfg, activation_cfg, weight_std, img_size, num_classes, in_chans,
        # embed_dims=[256,128,48,32], num_heads=[8, 4, 2], depths=[3, 8, 3], mlp_ratios=[4, 4, 4], sr_ratios=[2, 4, 6], encoder='medium')
