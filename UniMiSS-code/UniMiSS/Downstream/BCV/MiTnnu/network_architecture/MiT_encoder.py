...
def model_medium(norm_cfg='BN', activation_cfg='ReLU', weight_std=False, in_chans=1, **kwargs):
    model = MiT_encoder(norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std, in_chans=in_chans,
        embed_dims=[48,128,256,512], depths=[2, 3, 8, 3], num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4],
        sr_ratios=[6, 4, 2, 1], qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def model_large(norm_cfg='BN', activation_cfg='ReLU', weight_std=False, in_chans=1, **kwargs):
    model = MiT_encoder(norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std, in_chans=in_chans,
        embed_dims=[48,128,256,512], depths=[2, 6, 12, 3], num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4],
        sr_ratios=[6, 4, 2, 1], qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model