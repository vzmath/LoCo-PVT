...
# experimental with larger PVT-based encoder, ONLY change depths for now
# reference: https://github.com/whai362/PVT/blob/v2/classification/pvt.py#L255-L307
def encoder_medium(norm_cfg2D='BN2', norm_cfg3D='BN3', activation_cfg='ReLU', weight_std=False, img_size2D=224,
                  img_size3D=[16, 96, 96], modal_type='MM', **kwargs):
    model = MiT_encoder(norm_cfg2D=norm_cfg2D, norm_cfg3D=norm_cfg3D, activation_cfg=activation_cfg,
                              weight_std=weight_std, img_size2D=img_size2D,
                              img_size3D=img_size3D, embed_dims=[48, 128, 256, 512], depths=[2, 3, 8, 3],
                              num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4],
                              sr_ratios=[6, 4, 2, 1], qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                              modal_type=modal_type, **kwargs)
    return model

def encoder_large(norm_cfg2D='BN2', norm_cfg3D='BN3', activation_cfg='ReLU', weight_std=False, img_size2D=224,
                  img_size3D=[16, 96, 96], modal_type='MM', **kwargs):
    model = MiT_encoder(norm_cfg2D=norm_cfg2D, norm_cfg3D=norm_cfg3D, activation_cfg=activation_cfg,
                              weight_std=weight_std, img_size2D=img_size2D,
                              img_size3D=img_size3D, embed_dims=[48, 128, 256, 512], depths=[2, 6, 12, 3],
                              num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4],
                              sr_ratios=[6, 4, 2, 1], qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                              modal_type=modal_type, **kwargs)
    return model